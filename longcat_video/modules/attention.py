...
class Attention(nn.Module):
    ...
    def _process_attn(self, q, k, v, shape):
        ...
        if self.enable_bsa and shape[0] > 1: # bsa will not be used in image training / sampling
            ...
            # SDPA fallback (works on pre-Ampere GPUs such as the T4, where
            # flash-attn 2/3 are unavailable). q/k/v are [B, H, S, D].
            if q.device.type == "xla":
                # TPU/XLA: SDPA decomposes into math attention and would
                # materialize the full [B, H, S, S] score matrix. Dispatch to
                # the Pallas flash-attention kernel (chunked fallback).
                from longcat_video.xla_utils import spmd_self_attention
                x = spmd_self_attention(q, k, v, scale=self.scale)
            else:
...
class MultiHeadCrossAttention(nn.Module):
    ...
    def _process_cross_attn(self, x, cond, kv_seqlen):
        ...
        if self.enable_flashattn3:
            ...
            # SDPA fallback for the packed variable-length layout.
            # q: [1, B*N, H, D], k/v: [1, sum(kv_seqlen), H, D]
            head_dim = self.head_dim
            q_sd = q.view(B, N, self.num_heads, head_dim).permute(0, 2, 1, 3)  # [B, H, N, D]
            outs = []
            offset = 0
            for b in range(B):
                lb = int(kv_seqlen[b])
                k_b = k[:, offset:offset + lb].permute(0, 2, 1, 3)  # [1, H, lb, D]
                v_b = v[:, offset:offset + lb].permute(0, 2, 1, 3)
                o_b = F.scaled_dot_product_attention(q_sd[b:b + 1], k_b, v_b)  # [1, H, N, D]
                outs.append(o_b.permute(0, 2, 1, 3))  # [1, N, H, D]
                offset += lb
            x = torch.cat(outs, dim=0)  # [B, N, H, D]
