...
class LongCatVideoAvatarTransformer3DModel(
    ...
    def enable_loras(self, lora_key_list=[]):
        ...
        for module_name, loras in module_loras.items():
            ...
            # Place each LoRA on the SAME device as its target module. This is
            # required when the model is sharded across multiple GPUs.
            if hasattr(module, "weight_int8"):
                module_device = module.weight_int8.device
            else:
                try:
            ...
            for lora in loras:
                lora.to(module_device, dtype=model_dtype, non_blocking=True)
    ...
    # ------------------------------------------------------------------
    # Two-GPU model sharding ("pipeline parallelism" across cuda:0/cuda:1)
    # ------------------------------------------------------------------
    def shard_across_devices(self, devices=("cuda:0", "cuda:1"), split_index=None):
        """Place embedders/audio_proj/blocks[:split] on devices[0] and
        blocks[split:]/final_layer on devices[1]. The forward pass then hops
        the hidden states across the device boundary automatically.

        Call this AFTER all weights are materialized on their target devices
        (e.g. by ``load_quantized_dit_sharded``); in that case it only records
        the layout. If weights are all on one device, it will physically move
        them (requires enough free memory, so prefer the sharded loader).
        """
        ...
        self._shard_devices = devices
        self._shard_split_index = split_index
        self._block_devices = [devices[0] if i < split_index else devices[1] for i in range(depth)]

        def _ensure(module, device):
            try:
                p = next(iter(module.parameters()))
            except StopIteration:
                try:
                    p = next(iter(module.buffers()))
                except StopIteration:
                    return
            if p.device != device:
                module.to(device)

        for mod in (self.x_embedder, self.t_embedder, self.y_embedder, self.audio_proj):
            _ensure(mod, devices[0])
        for i, block in enumerate(self.blocks):
            _ensure(block, self._block_devices[i])
        _ensure(self.final_layer, devices[1])
        return self

    def _maybe_move_for_block(self, target_device, hidden_states, encoder_hidden_states, t, audio_hidden_states, token_ref_target_masks):
        if hidden_states.device != target_device:
            hidden_states = hidden_states.to(target_device, non_blocking=True)
            encoder_hidden_states = encoder_hidden_states.to(target_device, non_blocking=True)
            t = t.to(target_device, non_blocking=True)
            if audio_hidden_states is not None:
                audio_hidden_states = audio_hidden_states.to(target_device, non_blocking=True)
            if token_ref_target_masks is not None:
                token_ref_target_masks = token_ref_target_masks.to(target_device, non_blocking=True)
        return hidden_states, encoder_hidden_states, t, audio_hidden_states, token_ref_target_masks
    ...
    def forward(
        ...
        _input_device = hidden_states.device
        _block_devices = getattr(self, "_block_devices", None)
        ...
        kv_cache_dict_ret = {}
        for i, block in enumerate(self.blocks):
            if _block_devices is not None:
                hidden_states, encoder_hidden_states, t, audio_hidden_states, token_ref_target_masks = \
                    self._maybe_move_for_block(
                        _block_devices[i], hidden_states, encoder_hidden_states, t,
                        audio_hidden_states, token_ref_target_masks
                    )
            ...
            if return_kv:
                ...
                if offload_kv_cache:
                    ...
                    if hidden_states.device.type == "xla":
                        # XLA is lazy: .cpu() above only materializes the kv
                        # tensors; hidden_states' pending trace keeps growing,
                        # so block i's flush would compile a graph containing
                        # blocks 0..i (N unique, linearly growing graphs =
                        # O(N^2) compile work). Cutting here makes every block
                        # trace one identical, constant-size graph instead.
                        import torch_xla
                        torch_xla.sync()
                else:
        ...
        if _block_devices is not None:
            _final_device = next(self.final_layer.parameters()).device
            if hidden_states.device != _final_device:
                hidden_states = hidden_states.to(_final_device, non_blocking=True)
                t = t.to(_final_device, non_blocking=True)
        ...
        if _block_devices is not None and hidden_states.device != _input_device:
            hidden_states = hidden_states.to(_input_device, non_blocking=False)
