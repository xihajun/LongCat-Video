...
class LongCatVideoAvatarPipeline:
    ...
    def set_cached_text_embeddings(self, prompt_embeds, prompt_attention_mask,
                                   negative_prompt_embeds=None, negative_prompt_attention_mask=None):
        """Store pre-computed text embeddings so the text encoder can be freed."""
        self._cached_text_embeddings = (
            prompt_embeds, prompt_attention_mask,
            negative_prompt_embeds, negative_prompt_attention_mask,
        )

    def clear_cached_text_embeddings(self):
    ...
    def encode_prompt(
        ...
        # --- cached-embedding fast path -------------------------------------
        # ``set_cached_text_embeddings`` lets a caller pre-encode the prompt,
        # free the 11 GB umT5 text encoder, and still run the pipeline.
        cache = getattr(self, "_cached_text_embeddings", None)
        if cache is not None:
            c_pe, c_pm, c_npe, c_npm = cache
            c_pe = c_pe.to(device=device, dtype=dtype) if c_pe is not None else None
            c_pm = c_pm.to(device=device) if c_pm is not None else None
            if do_classifier_free_guidance:
                if c_npe is None:
                    raise ValueError("Cached text embeddings have no negative branch but CFG is enabled.")
                c_npe = c_npe.to(device=device, dtype=dtype)
                c_npm = c_npm.to(device=device)
                return c_pe, c_pm, c_npe, c_npm
            return c_pe, c_pm, None, None
        if self.text_encoder is None:
            raise ValueError(
                "text_encoder is None and no cached text embeddings were set. "
                "Call pipe.set_cached_text_embeddings(...) before freeing the text encoder."
            )
        # ---------------------------------------------------------------------
    ...
    def _vae_device(self):
        """Device the VAE actually lives on (may differ from the DiT device,
        e.g. VAE on host CPU while the DiT runs on TPU/XLA)."""
        try:
    ...
    def prepare_latents(
        ...
        if latents is not None:
            ...
            if isinstance(generator, torch.Generator) and generator.device.type != torch.device(device).type:
                latents = torch.randn(shape, generator=generator, device=generator.device, dtype=dtype).to(device)
            else:
            ...
            if need_encode:
                ...
                for i in range(batch_size):
                    ...
                    latent = retrieve_latents(
                        self.vae.encode(encoded_input.to(device=self._vae_device, dtype=self.vae.dtype)),
                        gen, sample_mode="argmax").to(encoded_input.device)
    ...
    def _cache_clean_latents(self, cond_latents, model_max_length, offload_kv_cache, device, dtype, audio_embs, num_cond_latents, num_ref_latents, ref_img_index):
        ...
        caption_channels = self.dit.config.caption_channels if self.dit is not None else self.text_encoder.config.d_model
    ...
    def generate_at2v(
        ...
        with tqdm(total=len(timesteps), desc="Denoising") as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if getattr(self, "_after_denoise_step", None) is not None:
                    self._after_denoise_step(i)
        ...
        latents = latents.to(device=self._vae_device, dtype=self.vae.dtype)
    ...
    def generate_avc(
        ...
        cond_videos_latents = retrieve_latents(
            self.vae.encode(cond_videos.to(device=self._vae_device, dtype=self.vae.dtype)),
            generator, sample_mode="argmax").to(cond_videos.device)
